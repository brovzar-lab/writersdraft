...
describe('Enter on empty element', () => {
  it('transforms empty character to action (escape hatch)', () => {
    expect(onEnter(ctx('character', ''))).toEqual({ kind: 'transform', type: 'action' })
  })
  it('transforms empty dialogue to action', () => {
    expect(onEnter(ctx('dialogue', ''))).toEqual({ kind: 'transform', type: 'action' })
  })
  it('empty action creates a new action (blank line flow)', () => {
    expect(onEnter(ctx('action', ''))).toEqual({ kind: 'create', type: 'action' })
  })
  it('transforms empty scene heading to action', () => {
    expect(onEnter(ctx('scene_heading', ''))).toEqual({ kind: 'transform', type: 'action' })
  })
})
...
describe('Backspace at start merges', () => {
  it('merges into previous element when caret is at 0', () => {
    expect(onBackspaceAtStart(ctx('dialogue', 'hello', 0))).toEqual({ kind: 'merge' })
  })
  it('does nothing mid-text', () => {
    expect(onBackspaceAtStart(ctx('dialogue', 'hello', 3))).toEqual({ kind: 'none' })
  })
  it('does nothing with a selection', () => {
    expect(
      onBackspaceAtStart({ element: makeElement('dialogue', 'hello'), caret: 0, hasSelection: true }),
    ).toEqual({ kind: 'none' })
  })
})

describe('Smart type detection', () => {
  it.each(['INT. HOUSE - DAY', 'EXT. STREET - NIGHT', 'INT./EXT. CAR - DUSK', 'I/E. TRAIN - DAY', 'EST. CITY SKYLINE'])(
    'detects scene heading: %s',
    (text) => {
      expect(detectTypeFromText(text, 'action')).toBe('scene_heading')
    },
  )
  it('detects lowercase int. prefix too', () => {
    expect(detectTypeFromText('int. kitchen - day', 'action')).toBe('scene_heading')
  })
...
describe('Normalisation', () => {
  it('uppercases scene headings, characters, transitions, shots', () => {
    expect(normalizeText('scene_heading', 'int. house - day')).toBe('INT. HOUSE - DAY')

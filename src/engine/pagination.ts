...
function paginateBlocks(
  ...
  const flushPageCarryingGroup = () => {
    ...
    while (cut > 0) {
      ...
      if (
        line.kind === "text" &&
        (line.type === "character" || line.type === "parenthetical")
      ) {
        cut--;
      } else {
        break;
  ...
  for (let i = startBlock; i < blocks.length; i++) {
    ...
    const isCuedGroupMember =
      b.element.type === "dialogue" || b.element.type === "parenthetical";
  ...
  if (current.length > 0 || (pages.length === 0 && pageOffset === 0))
    flushPage();
  return pages;
}
...
export function paginateIncremental(
  ...
  for (let p = 0; p < prev.pages.length - 1; p++) {
    ...
    if (DIALOGUE_GROUP.has(startType)) {
      ...
      while (groupEnd < old.length && DIALOGUE_GROUP.has(old[groupEnd].type))
        groupEnd++;

...
function importTitlePage(tp: XNode, script: Script): void {
  ...
  const byIdx = nonEmpty.findIndex((l) => /^(written\s+)?by$/i.test(l.text.trim()))
...
export function importFdx(xml: string): Script {
  ...
  if (!root) throw new Error('Not a Final Draft document (missing <FinalDraft> root)')
...
function paragraphXml(el: ScriptElement, script: Script, indent: string): string {

...
import { paginate, paginateIncremental, type PaginationCache } from './pagination'
...
const TYPES: ElementType[] = [
  'scene_heading', 'action', 'character', 'dialogue', 'parenthetical', 'transition', 'shot',
]
...
function randomScript(rnd: () => number, size: number): ScriptElement[] {
  ...
  for (let i = 0; i < size; i++) {
    ...
    const words = Math.floor(rnd() * (type === 'dialogue' || type === 'action' ? 60 : 6)) + 1
    out.push(makeElement(type, Array.from({ length: words }, (_, w) => `w${i}x${w}`).join(' ')))
  }
  return out
}

/** Apply one random structural edit, immutably (like the store does). */
function randomEdit(rnd: () => number, elements: ScriptElement[]): ScriptElement[] {
  const next = [...elements]
  const at = Math.floor(rnd() * next.length)
  const kind = rnd()
  if (kind < 0.4) {
    // text edit (typing)
    next[at] = { ...next[at], text: next[at].text + ' typed' + Math.floor(rnd() * 100) }
  } else if (kind < 0.6) {
    // insert
    next.splice(at, 0, makeElement(TYPES[Math.floor(rnd() * TYPES.length)], 'inserted text'))
  } else if (kind < 0.75) {
    // delete

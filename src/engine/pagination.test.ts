...
describe('paginate', () => {
  ...
  it('a short scene fits on one page', () => {
    const pages = paginate([scene(), action('Sunlight.'), character(), dialogue('Hello.')])
    expect(pages).toHaveLength(1)
  ...
  it('inserts spacing between blocks but never at the top of a page', () => {
    ...
    expect(lines.map((l) => l.kind)).toEqual(['text', 'blank', 'text', 'blank', 'text'])
  })

  it('never emits a page with more than LINES_PER_PAGE lines', () => {
    const els: ScriptElement[] = [scene()]
    for (let i = 0; i < 120; i++) els.push(action(`Beat ${i}. Something happens here.`))
  ...
  it('splits very long action blocks across pages without loss', () => {
    ...
    const textLines = pages.flatMap((p) => p.lines).filter((l) => l.kind === 'text' && l.elementId === longAction.id)

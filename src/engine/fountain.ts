...
const TRANSITION_EXACT = new Set([
  "FADE IN:",
  "FADE OUT.",
  "CUT TO BLACK.",
  "FADE TO BLACK.",
...
const SHOT_RE =
  /^(ANGLE ON|CLOSE ON|CLOSE UP|POV|INSERT|AERIAL SHOT|TRACKING SHOT|WIDE SHOT|EXTREME CLOSE UP)\b/;
...
function formatFountainLine(el: ScriptElement): string {
  switch (el.type) {
    ...
    case "transition": {
      const up = el.text.toUpperCase();
      return TRANSITION_RE.test(up) || TRANSITION_EXACT.has(up)
        ? up
        : `> ${up}`;
    }
    case "shot":
...
export function parseFountainBody(lines: string[]): ScriptElement[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith(".") && !line.startsWith("..")) {
      ...
      TRANSITION_EXACT.has(line.toUpperCase()) ||
      (TRANSITION_RE.test(line) && line === line.toUpperCase())
    ) {
      type = "transition";
      textOut = line.toUpperCase();
...
export function importFountain(text: string): Script {
  ...
  while (i < lines.length && tpRe.test(lines[i])) {
    ...
    else if (k === "author" || k === "authors" || k === "written by")
      script.titlePage.author = value.trim();

...
const SCENE_HEADING_PREFIXES = [
  'INT.', 'EXT.', 'INT/EXT.', 'INT./EXT.', 'EXT/INT.', 'EXT./INT.', 'I/E.', 'EST.',
]

const TRANSITION_SUFFIXES = ['TO:', 'OUT.', 'IN:', 'BLACK.']
const TRANSITION_EXACT = ['FADE IN:', 'FADE OUT.', 'CUT TO BLACK.', 'DISSOLVE TO:', 'SMASH CUT TO:', 'MATCH CUT TO:', 'FADE TO BLACK.']

const SHOT_PREFIXES = ['ANGLE ON', 'CLOSE ON', 'CLOSE UP', 'POV', 'INSERT', 'AERIAL SHOT', 'TRACKING SHOT', 'WIDE SHOT', 'EXTREME CLOSE UP']
 ...
 */
export function detectTypeFromText(text: string, current: ElementType): ElementType | null {
  const t = text.trimStart().toUpperCase()
  if (current === 'action' || current === 'general') {
    if (SCENE_HEADING_PREFIXES.some((p) => t.startsWith(p))) return 'scene_heading'

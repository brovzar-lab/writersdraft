...
const shortFdx = readFileSync(new URL('./__fixtures__/short.fdx', import.meta.url), 'utf8')
const dualFdx = readFileSync(new URL('./__fixtures__/dual.fdx', import.meta.url), 'utf8')
...
describe('FDX import', () => {
  it('imports the short fixture with correct types and exact text', () => {
    ...
    expect(s.elements.map((e) => e.type)).toEqual([
      ...
      'action', 'character', 'dialogue', 'transition', 'scene_heading', 'action',
    ])
  ...
  it('imports dual dialogue with left/right column marks', () => {
    ...
    expect(s.elements.filter((e) => !e.dual).every((e) => e.dual === undefined)).toBe(true)
  })

  it('imports script notes attached to the right element', () => {
    const s = importFdx(dualFdx)
    expect(s.notes).toHaveLength(1)
    expect(s.notes[0].text).toBe('Check with production: how many consoles fit on the set?')
  ...
  it('maps unknown paragraph types to general and rejects non-FDX XML', () => {
    ...
    expect(() => importFdx('<?xml version="1.0"?><NotFdx/>')).toThrow(/Final Draft/)
  })
})
...
describe('FDX export', () => {
  function sampleScript(): Script {
    ...
    s.notes = [
      { id: newId(), elementId: s.elements[1].id, author: 'Me', text: 'Too on the nose?', createdAt: 1 },
    ]
    return s
  }
  ...
  it('wraps contiguous dual-marked elements in one DualDialogue container', () => {
    ...
    expect(xml.indexOf('<DualDialogue>')).toBeGreaterThan(xml.indexOf('They speak at once.'))
    expect(xml.indexOf('</DualDialogue>')).toBeLessThan(xml.indexOf('Silence.'))
  })

  it('exports script notes inside their owning paragraph', () => {
    const xml = exportFdx(sampleScript())
    const para = xml.slice(xml.indexOf('Type="Action"'), xml.indexOf('Type="Character"'))
...
describe('FDX round trip', () => {
  ...
  it('exported XML from a round trip validates and keeps paragraph count', () => {
    ...
    expect(paragraphOpens.length).toBe(first.elements.length + first.notes.length)
  })
})

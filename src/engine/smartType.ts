...
export const TIMES_OF_DAY = [
  'DAY', 'NIGHT', 'MORNING', 'AFTERNOON', 'EVENING', 'DUSK', 'DAWN', 'CONTINUOUS', 'LATER', 'MOMENTS LATER', 'SAME TIME',
]
export const STANDARD_TRANSITIONS = [
  'CUT TO:', 'SMASH CUT TO:', 'MATCH CUT TO:', 'DISSOLVE TO:', 'FADE OUT.', 'FADE TO BLACK.', 'FADE IN:', 'INTERCUT WITH:',
]
export const CHARACTER_EXTENSIONS = ["(V.O.)", "(O.S.)", "(O.C.)", "(CONT'D)", '(INTO PHONE)', '(SUBTITLED)']
...
function prefixMatches(candidates: string[], typed: string, prefix = ''): Suggestion[] {
...
export function suggest(
  ...
  if (type === 'scene_heading') {
    ...
    if (!prefix) {
      ...
      return prefixMatches(SCENE_PREFIXES, up).map((s) => ({ ...s, text: s.text }))
    }
    ...
    if (dash >= 0) {
      ...
      return prefixMatches(TIMES_OF_DAY, typedTime, prefix + rest.slice(0, dash + 3))
    }
    if (rest.length === 0) {
      // Fresh location: offer known locations outright.
      return ctx.locations.slice(0, 6).map((l) => ({ text: prefix + l, completion: l }))
    }

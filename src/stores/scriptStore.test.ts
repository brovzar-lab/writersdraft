...
describe("scriptStore element CRUD", () => {
  ...
  it("inserts a new element after a given one", () => {
    ...
    expect(store().script.elements[1]).toMatchObject({
      type: "action",
      text: "Boom.",
    });
  ...
  it("merges an element into the previous one and reports the join point", () => {
    ...
    expect(store().script.elements[2].text).toBe(
      "SARAHWhat a beautiful morning.",
    );
...
describe("undo/redo", () => {
  ...
  it("new edits clear the redo stack", () => {
    ...
    expect(store().script.elements.filter((e) => e.text === "X")).toHaveLength(
      0,
    );
  });
});
...
describe("scenes and beat board", () => {
  ...
  it("stores scene color and synopsis", () => {
    const scenes = store().scenes();
    store().setSceneMeta(scenes[0].id, {
      color: "#f87171",
...
describe("tags, notes, alternates", () => {
  ...
  it("swaps alternate dialogue lines", () => {
    ...
    expect(store().script.elements[3].alternates).toContain(
...
describe("uiStore", () => {
  ...
  it("caret handoff request round-trips", () => {
    ...
    expect(useUiStore.getState().pendingCaret).toEqual({
      elementId: "el-1",
      offset: 5,
    });
...
describe("analysis engine", () => {
  it("counts speeches, words and scenes per character", () => {
    ...
    const stats = characterStats(s.elements, [
...
describe("fountain round-trip", () => {
  ...
  it("imports forced scene headings and transitions", () => {
    ...
    expect(s.elements[2]).toMatchObject({
      type: "transition",
      text: "SMASH CUT TO:",
    });
  });

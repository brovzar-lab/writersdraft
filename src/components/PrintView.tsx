...
function PrintLine({
  ...
  return (
    ...
    >
      {sceneNumber && (
        <>
          <span style={{ position: "absolute", left: "-0.9in" }}>
            {sceneNumber}
          </span>
          <span style={{ position: "absolute", right: "-0.7in" }}>
            {sceneNumber}
          </span>
        </>
      )}
...
export function PrintView() {
  ...
  for (const el of elements) {
    if (el.type === "scene_heading" && el.sceneNumber)
      sceneNumberOf.set(el.id, el.sceneNumber);
  }
  ...
  return (
    <div className="print-root font-screenplay">
      <section className="print-page">
        ...
        {titlePage.contact && (
          <div className="print-contact">{titlePage.contact}</div>
        )}
      </section>
      {pages.map((page) => (
        <section key={page.number} className="print-page">
          {page.number > 1 && (
            <span className="print-page-number">{page.number}.</span>
          )}

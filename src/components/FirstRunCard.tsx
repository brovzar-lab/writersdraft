...
function sampleElements() {
  return [
    ...
    makeElement("action", "A blank page glows on the monitor. YOU, a writer with a great idea, crack your knuckles."),
    makeElement("character", "MENTOR (V.O.)"),
    makeElement("parenthetical", "(warmly)"),
    makeElement("dialogue", "Tab and Enter. That's the whole trick. Enter moves the story forward, Tab changes what kind of line you're on."),
    ...
    makeElement("action", "Coffee cups everywhere. Page 90. You don't even look at the toolbar anymore."),
  ];
}
...
export function FirstRunCard() {
  ...
  const insertSample = () => {
    ...
    useUiStore.getState().requestCaret(parts[parts.length - 1].id, parts[parts.length - 1].text.length);
    dismiss();
  };
  ...
  return (
    <div
      ...
      className="no-print w-[34rem] max-w-[90vw] rounded-xl border border-line bg-desk-raised p-4 font-ui shadow-sm"
    >
      <h2 className="flex items-center gap-2 text-sm font-semibold text-ink">
        <IconZap size={14} className="text-brass" /> Write without touching the mouse
      </h2>
      <dl className="mt-3 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1.5 text-xs text-ink-soft">
        <dt><Key>Enter</Key></dt>
        <dd>next element — Scene Heading → Action, Character → Dialogue…</dd>
        <dt><Key>Tab</Key></dt>
        <dd>sibling element — Action → Character, Dialogue → Parenthetical…</dd>
        <dt><Key>int.</Key></dt>
        <dd>just type it — headings, transitions and shots are auto-detected</dd>
        <dt><Key>Ctrl K</Key></dt>
        <dd>jump to any scene or command</dd>
        <dt><Key>Ctrl P</Key></dt>
      ...
      <div className="mt-4 flex gap-2">
        ...
        <button
          onClick={dismiss}
          className="rounded-md border border-line px-3 py-1.5 text-xs text-ink-soft hover:border-line-strong hover:text-ink"
        >
          Got it — blank page
        </button>
      </div>
...
function Key({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="rounded border border-line bg-desk-sunken px-1.5 py-0.5 font-ui text-[10px] font-medium text-ink-soft">
      {children}
    </kbd>
  );
}

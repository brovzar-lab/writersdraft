...
function NavigatorDrawer() {
  ...
  return (
    ...
    >
      ...
      />
      <aside className="absolute inset-y-0 left-0 w-72 overflow-y-auto border-r border-line bg-bg font-ui shadow-2xl">
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const target =
        hashId ??
        (await getMeta<string>("currentScriptId").catch(() => null)) ??
        legacy?.id ??
        null;
      ...
      if (!stored && !hashId) {
        ...
        if (all.length > 0)
          stored = await getScript(all[0].id).catch(() => null);
      }
      if (cancelled) return;
  ...
  const performSave = useCallback(async () => {
    ...
    if (localOk && persistRequestRef.current === false) {
      ...
      void requestPersistentStorage().then((r) =>
        useUiStore.getState().setStoragePersist(r),
      );
    }
  ...
  const refreshUser = useCallback(() => {
    void (async () => {
      try {
        ...
        userIdRef.current = u?.uid ?? null;
        setUser(
          u ? { uid: u.uid, email: u.email, isAnonymous: u.isAnonymous } : null,
        );
      } catch {
        // ignore
      }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { loadScript: fetchCloudScript } =
        ...
        const state = useScriptStore.getState();
        if (
          state.script.id === target &&
          state.script.updatedAt === 0 &&
          !state.dirty
        ) {
  ...
  const openScriptById = useCallback(
    async (id: string) => {
      ...
      if (!stored) {
        window.alert(
          "Could not open that script — it is not on this device or in your cloud library.",
        );
        return;
      }
      const valid = migrateScript(stored);
  ...
  useEffect(() => {
    ...
    const onHash = () => {
      ...
      if (id && id !== useScriptStore.getState().script.id)
        void openScriptById(id);
    };
    ...
    <PaginationProvider>
      ...
      <div className={`app-root ${darkMode ? "dark" : ""}`}>
        <div className="flex h-screen flex-col bg-bg font-ui text-ink">
          ...
          {pendingRemote && (
            <div className="no-print flex items-center gap-3 border-b border-warning/40 bg-accent-soft px-3 py-2 font-ui text-sm text-ink">
              <span className="flex-1">
                A newer version of this script was saved from another session.
                Your unsaved local changes are safe — choose which version to
                keep. (Loading theirs keeps yours one Undo away.)
              </span>
              <button
                onClick={() => resolveConflict("take-remote")}
                className="rounded-md border border-warning/50 px-2 py-1 text-xs text-ink-2 hover:bg-sunken"
              >
                Load theirs
              </button>
              <button
                onClick={() => resolveConflict("keep-local")}
                className="rounded-md bg-accent px-2 py-1 text-xs text-on-accent hover:bg-accent-press"
              >
                Keep mine
              </button>
            </div>
                ...
                <>
                  ...
                  {view === "library" && (
                    <Library
                      ...
                      onDelete={(id) =>
                        void deleteLocalScript(id).catch(() => {})
                      }
                    />
                  )}
                </>
